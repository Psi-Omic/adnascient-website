...
(function () {
  ...
  for (let i = 0; i <= RUNG_COUNT; i++) {
    ...
    const nodeA = new THREE.Mesh(nodeGeo, new THREE.MeshBasicMaterial({ color: lilac, transparent: true, opacity: 0.2 }));
    nodeA.position.copy(pA);
    helixGroup.add(nodeA);
    const nodeB = new THREE.Mesh(nodeGeo, new THREE.MeshBasicMaterial({ color: teal, transparent: true, opacity: 0.2 }));
